...
Options:
    --input FILE    Input JSON file (default: test_outputs/sensor_test_data.json)
    --output FILE   Output FIT file (default: test_outputs/sensor_test_data.fit)
...
def create_fit_file(output_file: str, json_data: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        for hr_sample in heart_rate_data:
            timestamp = start_time + datetime.timedelta(seconds=float(hr_sample['timestamp']))
            fit_data.append({
                'type': 'record',
                'data': {
                    'timestamp': timestamp,
        ...
        for accel_sample in accel_data:
            timestamp = start_time + datetime.timedelta(seconds=float(accel_sample['timestamp']))
            fit_data.append({
                'type': 'record',
                'data': {
                    'timestamp': timestamp,
...
def parse_arguments():
    ...
    parser.add_argument(
        "--input", 
        type=str, 
        default=os.path.join(SCRIPT_DIR, "test_outputs", "sensor_test_data.json"),
        help="Input JSON file (default: test_outputs/sensor_test_data.json)"
    )
    
    parser.add_argument(
        "--output", 
        type=str, 
        default=os.path.join(SCRIPT_DIR, "test_outputs", "sensor_test_data.fit"),
        help="Output FIT file (default: test_outputs/sensor_test_data.fit)"
    )
